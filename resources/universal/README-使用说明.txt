...
- 所有分卷组成一个完整压缩包，只需从 .001 执行一次解压，软件会自动读取同目录的 .002、.003……；不需要逐个解压，不能只下载第一卷。文件名和编号请保持原样。
- 7-Zip 26.02 是本项目验证过的解压工具。使用 360 压缩等兼容软件时，从第一卷解压属于正常操作；须确认解压完成且没有缺卷、CRC 或数据错误。360 压缩各版本的 WIM/硬链接还原能力未经本项目逐一验证；若缺少模型文件、启动报错或占用空间异常，请使用 7-Zip 重新解压。不要用 Windows 资源管理器直接打开分卷。

...
(function () {
  ...
  ResultSession.prototype.build=function(){
    ...
    var transport=el("div","msr-transport");this.progress=el("input","msr-progress");this.progress.type="range";this.progress.min="0";this.progress.max=String(this.m.duration);this.progress.step=".01";this.progress.value="0";this.progress.oninput=function(){self.seek(parseFloat(this.value));};transport.appendChild(this.progress);transport.appendChild(el("span","msr-duration","/ "+this.m.duration.toFixed(1)+"s"));this.host.appendChild(transport);
  ...
  ResultSession.prototype.start=function(){var c=ctx(),self=this;if(this.position>=this.m.duration)this.position=0;window.dispatchEvent(new CustomEvent("music-to-midi-playback-start",{detail:{owner:this.ownerId}}));return c.resume().then(function(){if(self.disposed)return;self.stopSources();var startAt=c.currentTime+.02;self.startedAt=startAt-self.position;["original"].concat(self.m.instruments.filter(function(i){return i.detected;}).map(function(i){return i.id;})).forEach(function(id){var b=self.buffers[id];if(!b||self.position>=b.duration)return;var src=c.createBufferSource(),gain=c.createGain(),pan=c.createStereoPanner();src.buffer=b;src.connect(gain);gain.connect(pan);pan.connect(c.destination);src.start(startAt,self.position);self.sources.push(src);self.gains[id]=gain;self.panners[id]=pan;});self.playing=true;self.applyMix();self.play.textContent=self.m.strings.pause;self.tick();});};
  ...
  ResultSession.prototype.tick=function(){if(!this.playing)return;this.position=Math.min(this.m.duration,Math.max(this.position,ctx().currentTime-this.startedAt));if(this.position>=this.m.duration){this.position=this.m.duration;this.pause();return;}if(this.follow){var target=LEFT+this.position*this.pps-this.scroll.clientWidth/2;this.scroll.scrollLeft=clamp(target,0,Math.max(0,this.world.clientWidth-this.scroll.clientWidth));}this.layoutPlayhead();var self=this;this.raf=requestAnimationFrame(function(){self.tick();});};
  ...
  ResultSession.prototype.layoutPlayhead=function(){if(!this.playhead)return;var x=LEFT+this.position*this.pps-this.scroll.scrollLeft;this.playhead.style.transform="translate3d("+x.toFixed(2)+"px,0,0)";this.playhead.style.visibility=(x>=LEFT&&x<=this.scroll.clientWidth)?"visible":"hidden";this.clock.textContent=this.position.toFixed(1)+"s";if(this.progress)this.progress.value=String(this.position);};

...
- **人声分离模式**：BS-RoFormer 分离人声与伴奏，分别转写为独立 MIDI 文件（默认 checkpoint：`model_bs_roformer_ep_368_sdr_12.9628.ckpt`）
...
| BS Roformer SW（6-stem） | [python-audio-separator model-configs](https://github.com/nomadkaraoke/python-audio-separator/releases/tag/model-configs) / [MVSEP News](https://www.mvsep.com/news) | 开源可下载（6-stem） + API | ✅ 可下载（非 2-stem 直替） | 同名 checkpoint（`BS-Roformer-SW.ckpt`）已公开；但它是 6-stem 方向，接入本项目需改推理/后处理流程 |

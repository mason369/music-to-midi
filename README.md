...
桌面版、Gradio Web 版和 Google Colab 采用同一套七模式工作流与操作语义。以下演示按“主界面 → 分离完成 → 逐轨处理”的顺序展示核心流程。
...
- **人声/伴奏分离转写**：`VOCAL_SPLIT` 模式用 Leap XE 90-band 提取 vocals、PolarFormer 提取 accompaniment，再分别运行所选 YourMT3+ / MIROS；也可额外输出合并 MIDI。
- **六声部分离转写**：`SIX_STEM_SPLIT` 模式用 `BS-Rofo-SW-Fixed.ckpt` 分离 `bass / drums / guitar / piano / vocals / other`，每个 stem 独立运行所选 YourMT3+ / MIROS，再输出各 stem MIDI 和合并 MIDI。
...
- **默认后端语义**：多乐器默认后端为 YourMT3+ 官方 `YPTF.MoE+Multi (noPS)`；`SMART`、`VOCAL_SPLIT`、`SIX_STEM_SPLIT` 都可显式切换 MIROS。
- **MIROS 可选后端**：三个多乐器工作流都可切换到本地 `ai4m-miros` 仓库作为实验性后端。
- **官方音符与 tempo**：三个多乐器模式都直接保留 YourMT3+ / MIROS 官方 writer 的音符、音色、力度、控制器和弯音消息，不再执行项目级量化、去重、稀疏乐器过滤、短音符过滤、力度平滑、复音限制或 NoteEvent 重建。若官方文件没有 `set_tempo`，只按检测 BPM 保持绝对秒并补写 tempo；检测失败会明确停止。
...
| 入口 | 处理模式 | 后端选择 | 适合场景 |
|------|----------|----------|----------|
| PyQt6 桌面版 | `SMART`、`VOCAL_SPLIT`、`SIX_STEM_SPLIT`、`PIANO_TRANSKUN`、`PIANO_TRANSKUN_V2_AUG`、`PIANO_ARIA_AMT`、`PIANO_BYTEDANCE_PEDAL` | 三个多乐器模式默认 YourMT3+ noPS，可选 MIROS；钢琴模式使用各自固定后端 | 本地长期使用、GPU 推理、批量输出文件、钢琴专用转写 |
| Gradio Space | 同桌面七种模式 | 三个多乐器模式可选 YourMT3+ / MIROS 及 YourMT3 checkpoint；钢琴模式使用各自固定后端 | 浏览器中快速试用或部署 |
| Google Colab | 同桌面七种模式 | 三个多乐器模式可选 YourMT3+ / MIROS 及 YourMT3 checkpoint；钢琴模式使用各自固定后端 | 临时使用 Colab GPU |
...
- 桌面版、Gradio Space 和 Colab 都暴露同一组七种处理工作流；三个需要多乐器转写的工作流会再选择 YourMT3+ / MIROS，选择 YourMT3+ 时还可选择官方 checkpoint 模式。
...
| `colab_notebook.ipynb` | 保留 Colab 预装 Torch，安装 pinned Web/runtime 依赖，并同步七种模式 | 三个多乐器模式都暴露 YourMT3+ / MIROS 与 YourMT3 checkpoint 选择器。 |

## 处理模式

| 模式 | 内部流程 | 主要输出 | 说明 |
|------|----------|----------|------|
| `SMART` | 音频 -> YourMT3+ / MIROS 官方 writer -> tempo 元数据 | `<歌曲名>.mid` | 不做音源分离，不做项目级音符清理；适合大多数混音歌曲、纯音乐和多乐器片段。 |
| `VOCAL_SPLIT` | 音频 -> Leap XE 90-band vocals + PolarFormer accompaniment -> 两路分别运行所选 YourMT3+ / MIROS -> MIDI 生成 | `<歌曲名>_accompaniment.mid`、`<歌曲名>_vocal.mid`，可选 `<歌曲名>_vocal_accompaniment_merged.mid` | 两个分离模型各自负责目标 stem；MIDI 阶段对 vocals 与 accompaniment 分别执行所选后端。 |
| `SIX_STEM_SPLIT` | 音频 -> `BS-Rofo-SW-Fixed.ckpt` 六声部 WAV 分离 -> 每个 stem 独立运行所选 YourMT3+ / MIROS -> stem MIDI 合并 | `<歌曲名>_<stem>.mid`、`<歌曲名>_all_stems_merged.mid` | 不再对原混音只跑一次 AMT，也不按 GM 乐器族路由；六个 stem 都走同一个用户显式选择的多乐器后端。 |
...
实际文件数量取决于所选模式、是否启用合并 MIDI，以及是否保留分离 WAV。人声分离只产生规范的 `<歌曲名>_vocals.wav` 与 `<歌曲名>_accompaniment.wav`。六声部模式必须完整生成并独立转写六个 stem；缺少任一 stem 或任一路转写失败时整体失败，不发布部分合并结果。
...
|-----------|------|----------|--------------|----------|
| YourMT3+ | 多乐器 AMT | 三个多乐器模式都直接采用官方 writer 音符；`VOCAL_SPLIT` 对两个 stem、`SIX_STEM_SPLIT` 对六个 stem 分别推理，随后只补 tempo/合并元数据 | 官方 Space 默认 noPS 结果文件：Slakh `multi_f = 0.7398`；YourMT3+ 论文表：Slakh2100 `Multi F1 = 74.84`，同表 `MT3 = 62.0` | 默认多乐器后端；工程默认 checkpoint 为 `YPTF.MoE+Multi (noPS)`。 |
| MIROS | 多乐器 AMT | `SMART`、`VOCAL_SPLIT`、`SIX_STEM_SPLIT` 可选多乐器后端 | 上游仓库标注为 2025 AI4Musician winning model | 实验性本地后端，适合与 YourMT3+ 做同任务 A/B。 |
...
YourMT3+ / MIROS 属于多乐器后端，TransKun / Aria-AMT / ByteDance Pedal 属于钢琴专精后端，Leap XE / PolarFormer / BS-RoFormer SW Fixed 属于音源分离后端；三类模型的公开指标不能混成同一张排行榜。
...
`VOCAL_SPLIT` 的模型与输入输出契约对齐当前公开 TelkNet 工具：BS-RoFormer Leap XE 90-band 对原混音生成 vocals，BS PolarFormer public ONNX 也对原混音独立生成 accompaniment。两路规范 WAV 随后分别交给用户选择的 YourMT3+ 或 MIROS。

对齐边界：本轮经授权核验了私有 `mason369/telknet` 的 `dev` 提交 `52be6fec179be492f5229ba149545ac2833b284a`。当前工程对齐其 YourMT3/MIROS“官方 writer 后只补 tempo、不做通用音符清理”的核心语义；但桌面版仍保留自身七模式产品契约，`VOCAL_SPLIT` 与 `SIX_STEM_SPLIT` 会继续逐 stem 生成 MIDI，而私有 dev 的两个独立工具是 WAV-only。没有证据证明该 `dev` 已部署线上，也不声称模式路由逐行一致、推理环境相同或输出文件位级一致。
...
| 输出选项 | 分离阶段输出 `<歌曲名>_vocals.wav` 与 `<歌曲名>_accompaniment.wav`；MIDI 阶段输出两路 MIDI，可选额外输出 1 个合并 MIDI |
...
|------|--------------|---------------|----------------|---------------------------------------|
| 六声部分离 + 分别转写 | `six_stem_split` 已在 pipeline、桌面 UI、Space 和 Colab 中开放 | `BS-Rofo-SW-Fixed.ckpt`（vocals, bass, drums, guitar, piano, other）+ 每个 stem 独立运行所选 YourMT3+ / MIROS | MVSEP Algorithms #77 给出 6-stem SDR：vocals 11.30 / instrum 17.50 / bass 14.62 / drums 14.11 / guitar 9.05 / piano 7.83 / other 8.71 | 这些是音源分离 SDR，不是最终 MIDI 转写 F1；逐 stem AMT 的端到端质量没有公开统一 benchmark。 |
...
趋势总结：截至 2026 年初，多乐器 AMT 的公开强路线主要沿两条线发展：一条是 `MT3 / YourMT3+ / MR-MT3` 这种 token-based 专用模型演进，另一条是 `MusicFM` 这类预训练编码器增强路线。钢琴 AMT 的公开成熟度仍然更高，但 `TransKun pip 权重`、`V2 Aug checkpoint`、`论文 checkpoint`、`pedal-aware 论文系统` 之间的协议差异必须写清楚，不能简单合并成一个“钢琴榜单”。
...
- FFmpeg / 音频解码失败。
- YourMT3+、MIROS、TransKun 默认 V2 / V2 Aug、Aria-AMT、ByteDance Pedal 或分离模型推理失败。
- 分离器没有生成完整目标文件。
- 模型下载遇到证书、网络或完整性问题。
...
`requirements.txt` 有意不让 `audio-separator` 的 NumPy 2 元数据覆盖桌面 NumPy 1.26，也不让 Aria-AMT 的旧 torchaudio 约束覆盖当前 Torch。上述两项因此必须按固定版本以 `--no-deps` 单独安装；需要完整伴随依赖时优先运行 `install.ps1` / `install.sh`。

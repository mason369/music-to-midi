...
Chord colors identify roots and the current interval follows the playhead. Short intervals keep full labels in staggered callouts anchored to their true timing. Desktop, Space and Colab highlight MIDI notes at the playhead and show pitch, instrument, velocity and timing on hover. Muted instruments do not receive playback highlights; editing selections remain independent.
...
`v1.6.0` delivers Issue #9 as a complete Docker self-host package with model-free backend and gateway images, a digest-pinned Compose file, an environment template, Windows/Linux management scripts, image identities, and SHA-256 checksums. The default address is `http://127.0.0.1:7860`; selected model profiles are stored in a persistent volume, and the long-running inference container stays offline. The [Docker self-host guide](docker-deployment.en.md) covers installation, configuration, logs, backup, upgrades, and troubleshooting.

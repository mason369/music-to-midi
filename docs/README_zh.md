...
- **人声/伴奏分离转写**：`VOCAL_SPLIT` 模式用 BS-RoFormer Leap XE 90-band 提取 vocals、BS PolarFormer 提取 accompaniment，再对两路分别运行用户选择的 YourMT3+ / MIROS；可选额外输出一个合并 MIDI。
- **六声部分离转写**：`SIX_STEM_SPLIT` 模式用 `BS-Rofo-SW-Fixed.ckpt` 分离 `bass / drums / guitar / piano / vocals / other` 六个真实 stem，每个 stem 独立运行用户选择的 YourMT3+ / MIROS，再输出各 stem MIDI 和合并 MIDI。
...
- **默认后端语义**：多乐器默认后端为 YourMT3+ 官方 `YPTF.MoE+Multi (noPS)`；`SMART`、`VOCAL_SPLIT`、`SIX_STEM_SPLIT` 都可显式选择 YourMT3+ / MIROS，选择 YourMT3+ 时可进一步选择五种官方 checkpoint。
- **MIROS 可选后端**：桌面版、Space 和 Colab 的三个多乐器模式都可切换到本地 `ai4m-miros` 仓库作为实验性后端。
- **官方音符与 tempo**：三个多乐器模式都直接保留 YourMT3+ / MIROS 官方 writer 的音符、音色、力度、控制器和弯音消息，不执行项目级量化、去重、稀疏乐器/短音符过滤、力度平滑、复音限制或 NoteEvent 重建。官方文件缺少 `set_tempo` 时，只按检测 BPM 保持绝对秒并补写 tempo；检测失败会明确停止。
...
| 入口 | 处理模式 | 后端选择 | 适合场景 |
|------|----------|----------|----------|
| PyQt6 桌面版 | `SMART`、`VOCAL_SPLIT`、`SIX_STEM_SPLIT`、`PIANO_TRANSKUN`、`PIANO_TRANSKUN_V2_AUG`、`PIANO_ARIA_AMT`、`PIANO_BYTEDANCE_PEDAL` | 三个多乐器模式显式选择 YourMT3+ / MIROS；选择 YourMT3+ 时可选五种官方 checkpoint；钢琴模式按入口使用固定后端 | 本地长期使用、GPU 推理、批量输出文件、钢琴专用转写 |
| Gradio Space | 同桌面七种模式 | 三个多乐器模式都提供 YourMT3+ / MIROS 与 YourMT3 checkpoint 选择 | 浏览器中快速试用或部署 |
| Google Colab | 同桌面七种模式 | 三个多乐器模式都提供 YourMT3+ / MIROS 与 YourMT3 checkpoint 选择 | 临时使用 Colab GPU |
...
| `VOCAL_SPLIT` | 音频 -> Leap XE 90-band vocals + PolarFormer accompaniment -> 两路分别运行所选 YourMT3+ / MIROS -> MIDI 生成 | `<歌曲名>_accompaniment.mid`、`<歌曲名>_vocal.mid`，可选 `<歌曲名>_vocal_accompaniment_merged.mid` | 两个分离模型独立负责各自目标 stem；任一分离或转写失败都会停止流程。 |
| `SIX_STEM_SPLIT` | 音频 -> `BS-Rofo-SW-Fixed.ckpt` 六声部 WAV 分离 -> 每个 stem 独立运行所选 YourMT3+ / MIROS -> stem MIDI 合并 | `<歌曲名>_<stem>.mid`、`<歌曲名>_all_stems_merged.mid` | 每个 stem MIDI 都直接来自对应 WAV 的独立转写；六个真实 stem 使用同一个用户显式选择的多乐器后端。 |
...
实际文件数量取决于所选模式、是否启用人声分离合并 MIDI，以及是否保留分离 WAV。人声分离对外只暴露规范的 `vocals` 与 `accompaniment`；六声部模式基于六个真实分离 WAV 分别生成 MIDI。
...
本节恢复自历史 README 中的模型对比内容，并按当前版本的实际能力重新标注：当前已发布入口同步开放 `SMART`、`VOCAL_SPLIT`、`SIX_STEM_SPLIT`、`PIANO_TRANSKUN`、`PIANO_TRANSKUN_V2_AUG`、`PIANO_ARIA_AMT` 与 `PIANO_BYTEDANCE_PEDAL` 七种模式。下列表格把“公开 benchmark”和“项目内入口状态”分开写，避免把研究指标误写成产品能力。
...
TelkNet 边界：本轮经授权核验了私有 `mason369/telknet` 的 `dev` 提交 `52be6fec179be492f5229ba149545ac2833b284a`。当前工程对齐其 YourMT3/MIROS“官方 writer 后只补 tempo、不做通用音符清理”的核心语义；但桌面版保留自身七模式契约，`VOCAL_SPLIT` 与 `SIX_STEM_SPLIT` 会继续逐 stem 生成 MIDI，而私有 dev 的两个独立工具是 WAV-only。没有证据证明该 `dev` 已部署线上，也不声称模式路由逐行一致、推理环境相同或输出文件位级一致。
...
| 输出选项 | 分离阶段输出规范的 `vocals` 与 `accompaniment`；MIDI 阶段分别转写两路，可选额外输出一个合并 MIDI |
...
|------|--------------|---------------|----------------|---------------------------------------|
| 六声部分离 + 分别转写 | `six_stem_split` 已在 pipeline、桌面 UI、Space 和 Colab 中开放 | `BS-Rofo-SW-Fixed.ckpt`（vocals, bass, drums, guitar, piano, other）+ 每个真实 stem 独立运行所选 YourMT3+ / MIROS | MVSEP Algorithms #77 给出 6-stem SDR：vocals 11.30 / instrum 17.50 / bass 14.62 / drums 14.11 / guitar 9.05 / piano 7.83 / other 8.71 | 这些是音源分离 SDR，不是最终 MIDI 转写 F1；逐 stem AMT 的端到端质量没有公开统一 benchmark。 |
...
|------|----------------|---------------------|----------|
| Windows / NVIDIA 桌面与便携目标 | Python 3.11-3.12；Torch 2.7.0 / torchaudio 2.7.0 / torchvision 0.22.0 | NumPy 1.26.4；CUDA 12.8 wheel | 源码启动器按此契约校验；当前 `release.yml` 因第三方再分发许可未清零而不生成便携成品 |
...
> 当前发布状态：仅保留并验证未来便携构建流程；`THIRD_PARTY_NOTICES.md` 中仍有模型权重、补丁源码和运行时再分发许可未解决，官方 release 门禁会显式阻断。下面的本地构建命令不等于获得再分发授权。
